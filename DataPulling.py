...
for x in range(len(split['train']['Drug1'])):
    mol1 = Chem.MolFromSmiles(split['train']['Drug1'][x])
    mol2 = Chem.MolFromSmiles(split['train']['Drug2'][x])
    if mol1 == None or mol2 == None:
        pass
    else:
        y_train.append(split['train']['Y'][x])
...
#np.savetxt("rdk_fingerprint.txt", output, fmt='%i')

#output = np.hstack((drug1_ap, drug2_ap))
#np.savetxt("ap_fingerprint.txt", output, fmt='%i')

#output = np.hstack((drug1_tt, drug2_tt))
#np.savetxt("tt_fingerprint.txt", output, fmt='%i')

#output = np.hstack((drug1_morgan, drug2_morgan))
#np.savetxt("morgan_fingerprint.txt", output, fmt='%i')

#output = np.hstack((drug1_avalon, drug2_avalon))
#np.savetxt("avalon_fingerprint.txt", output, fmt='%i')

#output = np.hstack((drug1_erg, drug2_erg))
#np.savetxt("erg_fingerprint.txt", output, fmt='%i')

...
x = np.loadtxt("all.txt", dtype=int)
x_test = torch.tensor(np.loadtxt("all_test.txt"))

N_SAMPLES, D_in, D_out = 134147, 300, 86
...
model = torch.nn.Sequential(
    torch.nn.Linear(D_in, 128).double(),
    torch.nn.ReLU().double(),
    torch.nn.Linear(128, D_out).double(),
)

...
x = np.loadtxt("erg_fingerprint.txt", dtype=int)
x_test = torch.tensor(np.loadtxt("erg_fingerprint_test.txt")).double()

N_SAMPLES, D_in, D_out = 134147, 630, 86
...
model = torch.nn.Sequential(
    torch.nn.Linear(D_in, 256).double(),
    torch.nn.ReLU().double(),
    torch.nn.Linear(256, D_out).double(),
)
